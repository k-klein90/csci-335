...
class Folder {
   ...
   public:
       ...
       * @throw InvalidFormatException if the name is invalid (e.g. contains non-alphanumeric characters)
       */
       ...
       */
      bool rename(const std::string &new_foldername);

      /**
       * @brief Sorts and prints the names of subfolder and file vectors lexicographically (i.e. alphabetically).
       ...
       *     ...
       *    <FILENAME_N>
       * @note: This CAN be done more efficiently by maintaining sorted order in the vectors already, instead of sorting
       * each time we print. However, we'll hold off on that for now, since we just want to get used to iterating with
       * iterators.
       */
      void display();
      ...
      /**
       * @brief Appends the given file to the files_ vector using move_semantics on the parameter File object, if a file
       * with the same name does not exist within the files_ vector. (HINT!) Consider push_back(). What happens when we
       * give it an l-value vs. an r-value? Does it change anything?
       *
       * @param new_file A reference to a File object to be added.
       *    - If the name of the File object is empty (i.e. its contents have been taken via move) the add fails
       ...
       *    - If a matching name is found, use move semantics to move the object from the current directory to the file
       *    vector within the destination folder and erase it from the current folder.
       *    - If a matching name is not found within the source folder or an object with the same name already exists
       *    within the destination folder, nothing is moved.
